...
def main():
    ...
    print(args.audio)
    files=os.listdir(args.audio)
    #print(files)
    files=sorted(files)
    print(args.nums)
    for i in range(0,args.nums): 
        fin = wave.open(args.audio+files[i], 'rb')
        ...
        if args.extended:
            ...
            out.write(metadata_to_string(ds.sttWithMetadata(audio, 1).transcripts[0]))
            out.write("0")
        elif args.json:
            print(metadata_json_output(ds.sttWithMetadata(audio, args.candidate_transcripts)))
            out.write(metadata_json_output(ds.sttWithMetadata(audio, args.candidate_transcripts)))
            out.write("1")
        else:
            print(ds.stt(audio))
            out.write(ds.stt(audio))
            out.write("\n")
